...
export function personActions(person, { onOpen, onLog, onEdit } = {}) {
  const actions = []
  if (onOpen) actions.push({ label: 'Open profile', icon: ArrowRight, onClick: () => onOpen(person.id) })
  ...
  if (person.phone) actions.push({ label: 'Call', icon: Phone, onClick: () => { window.location.href = `tel:${person.phone}` } })
  if (person.email) actions.push({ label: 'Email', icon: Mail, onClick: () => { window.location.href = `mailto:${person.email}` } })
  return actions
}
