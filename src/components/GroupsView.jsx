...
export default function GroupsView({ data, openId, onOpenPerson, onOpenTask, onAdd, onEdit, isDemo = false }) {
  const { groups, people, orgs, tasks, taskLinks, addTask, addTaskLink, saveGroup, loading, deleteGroup } = data
    ...
    <div>
      ...
      {groups.length === 0 ? (
        ...
        <div className="list">
          {groups.map((group) => {
            ...
            const linkedOpen = linkedTasksFor('group', group.id, tasks, taskLinks).filter((t) => !t.completed_at)
              ...
              <div key={group.id}>
                <div className="list-row" onClick={() => setExpandedId(expanded ? null : group.id)}>
                  ...
                  {expanded ? (
                    ...
                    <Avatar name={group.name} src={group.avatar_url} kind="group" icon={Users} size={42} />
                  )}
                ...
                {expanded && (
                  <div style={{ padding: '4px 14px 14px 68px', background: 'var(--surface)', borderBottom: '0.5px solid var(--separator)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 16 }}>
                      ...
                      <button className="text-btn" onClick={() => setLinkingGroup(group)}>Link a task</button>
                      {members.length > 0 && (
                        <button className="text-btn" onClick={() => downloadVcf(group.name, members, orgsById)}>
                          Export contacts
                        </button>
                      )}
                      <button
      ...
      {linkingGroup && (
        <LinkTaskForm
          ...
          tasks={tasks}
          existingTaskIds={new Set(linkedTasksFor('group', linkingGroup.id, tasks, taskLinks).map((t) => t.id))}
          addTask={addTask}
