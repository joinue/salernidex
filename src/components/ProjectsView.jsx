...
export default function ProjectsView({ data, onOpenProject, onAdd, onSearch, hub, sort, onSort }) {
  ...
  const renderProject = (p) => {
    ...
    const sub = range || (next ? `Next: ${next.title}` : progress ? `${progress.total} tasks` : 'No tasks yet')
  ...
  return (
    <div>
      ...
      {total === 0 ? (
        <div className="empty">
          <Folder size={28} className="empty-icon" />
        ...
        <>
          {total > 1 && (
            <Segmented options={SORT_OPTIONS} value={sort} onChange={onSort} />
          )}
