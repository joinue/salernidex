  ...
  if (!habit) {
    return (
      <div>
        ...
        <p className="empty">Habit not found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="habit-detail-page">
      ...
      <header className="habit-detail-head">
        ...
        {!readOnly && (
          <button className="header-action neutral" onClick={() => onEdit(habit)} aria-label="Edit">
            <Edit2 size={18} />
          </button>
        )}
      </header>
      ...
      <div className="insight-grid">
        {isTrack ? (
          <>
            <Insight label="30-day avg" value={s30.average.toFixed(1)} />
            <Insight label="Logged" value={`${t.scheduled}`} sub="days" />
            {!tr.young && <Insight label="Trend" value={<TrendIcon dir={tr.dir} />} sub={tr.dir} />}
          </>
        ) : weekly ? (
          <>
            {habit.track_streak && <Insight label="Streak" value={`${streak}`} sub="weeks" />}
            <Insight label="This week" value={`${wp.count}/${wp.target}`} />
            <Insight label="Days done" value={`${t.successes}`} sub="total" />
            {!tr.young && <Insight label="Trend" value={<TrendIcon dir={tr.dir} />} sub={tr.dir} />}
          </>
        ) : (
          <>
            {habit.track_streak && <Insight label="Streak" value={`${streak}`} sub="days" />}
            {habit.track_streak && (
              <Insight label="Best" value={`${bestStreak(habit, map, today)}`} sub="days" />
            )}
            <Insight label="30-day" value={`${pct(s30)}%`} />
            <Insight label="All-time" value={`${allTimePct}%`} />
            <Insight label={doneLabel} value={`${t.successes}`} sub="days" />
            {best && (
              <Insight
                label="Best day"
                value={DOW_FULL[best.dow].slice(0, 3)}
                sub={`${Math.round(best.rate * 100)}%`}
              />
            )}
            {!tr.young && <Insight label="Trend" value={<TrendIcon dir={tr.dir} />} sub={tr.dir} />}
          </>
      ...
      {/* Chart */}
      <div className="habit-section">
        <span className="section-label">{isTrack ? 'Trend' : 'By week'}</span>
