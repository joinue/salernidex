...
export default function ActivityRow({ entry: e, onOpenPerson, onOpenList, onOpenTasks, onPersonLongPress }) {
  if (e.kind === 'completion') {
    return (
      <PressableRow key={e.key} onClick={onOpenTasks}>
        <span className="feed-check"><CheckCircle size={20} /></span>
    ...
    return (
      <PressableRow key={e.key} onClick={() => onOpenList(e.list.id)}>
        ...
        <div className="row-body">
          ...
          <div className="row-sub">{e.action === 'checked' ? `Checked off ${e.text}` : `Added ${e.text}`}</div>
        </div>
        <span className="row-time">{relativeTime(e.ts)}</span>
      </PressableRow>
    )
