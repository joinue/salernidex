...
- **Lists** — shared groceries/hardware/packing, rapid add, check-off, "Got it" section; list types + sections, per-item qty/assignee/note/due, aisle ordering ([`lib/aisles.js`](src/lib/aisles.js)), per-list color (`0031`), and a learned **catalog** for fast re-add.
...
- **Notebook tier 2** — note-to-note links, search match highlighting, multi-select, rail/Back behavior. All four unbuilt. [`docs/scopes/notes.md`](docs/scopes/notes.md).
